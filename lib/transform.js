...
const GEMSTONE_MAP = {
  ...
  oxzir: 'Cubic Zirconia', oxyde: 'Cubic Zirconia',
...
const STYLE_MAP = [
  ...
  ['identit',        'ID'],             ['jonc',          'bangle'],
  ['creole',         'hoop'],           ['puces',         'studs'],
  ['multi',          'multi-stone'],    ['empierr',       'stone-set'],
  ['godron',         'fluted'],         ['torsad',        'twisted'],
  ['alternee',       'alternating link'], ['maille',      'chain'],
...
const SPECIFIC_CHAIN_TYPES = new Set(['anchor chain', 'ball chain', 'fancy link', 'alternating link', 'flexible', 'curb chain']);
...
function getGemstone(supp) {
  ...
  const caratM   = (supp || '').match(/(\d+[.,]\d+)\s*ct/i);
  const qualityM = (supp || '').match(/\b([A-Z]{1,2}-[A-Z]{1,2})\b/);
  const details  = [];
...
function getStyle(libProd, libSupp) {
  ...
  const n = norm(afterColon);
  const seen = [];
  for (const [key, val] of STYLE_MAP) {
    if (n.includes(key) && !seen.includes(val)) seen.push(val);
  }
  ...
  const sizeM   = (libProd || '').match(/(\d+)\s*mm/i);
...
function buildDescription(r) {
  ...
  const gemstone  = getGemstone(libSupp);
