...
export function getExtraTags(r) {
  ...
  // Both the raw label and the derived gemstone are checked so either spelling
  // ("Dia.Syn" or "Diamant … Syn") is caught.
  if (/dia[.\s-]*syn/.test(labelN) || /Synthetic Diamond/.test(gemstone)) tags.push('Lab Diamonds');
  if (/dia[.\s-]*nat/.test(labelN) || /Natural Diamond/.test(gemstone))   tags.push('Natural Diamonds');
  if (labelN.includes('enfant')) tags.push('Kids');
...
// Minimum columns to update Description and Tags on existing Shopify products without
// touching inventory, pricing, or images. Both are product-level fields, but Option
...
function transform(rows) {
  return rows
    ...
    // Step 17b: derive merchandising tags from the French catalogue label
    .map(r => {
      const extra = getExtraTags(r);
      return extra.length ? { ...r, Tags: r.Tags + ', ' + extra.join(', ') } : r;
    })
      ...
      complete: ({ data }) => {
        ...
        setTimeout(() => {
          try {
            ...
            // Description + Tags update files: all variant rows, 8 columns.
            ...
            const descFiles = descChunks.map((rows, i) => ({
              label:    `Description + Tags Update — Part ${i + 1} of ${descChunks.length}`,
