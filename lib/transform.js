...
export function getShopifySKUs(rows) {
  ...
  const skus = new Set(rows.map(row => canonicalSKU(row[skuColumn])).filter(Boolean));
  if (!skus.size) throw new Error('The Shopify export does not contain any variant SKUs.');
  return skus;
}
...
const SHOPIFY_COMPARE_FIELDS = [
  { output: 'Description', sources: ['Body (HTML)', 'Description'], normalize: normalizeText },
...
export async function processFile(file, shopifyFile = null) {
  const [encoding, shopifyEncoding] = await Promise.all([
    detectEncoding(file),
    shopifyFile ? detectEncoding(shopifyFile) : Promise.resolve(null),
  ]);
  const [mrmParsed, shopifyParsed] = await Promise.all([
    parseCSVFile(file, encoding),
    shopifyFile ? parseCSVFile(shopifyFile, shopifyEncoding) : Promise.resolve(null),
  ]);
  ...
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      try {
            ...
            if (shopifyParsed) {
              const shopifySKUs = getShopifySKUs(shopifyParsed.data);
              shopifyVariants = shopifySKUs.size;
              sourceRows = filterMRMRowsToShopifyVariants(data, shopifySKUs)
                .map(row => ({ ...row, _shopifyExistingVariant: true }));
              dropReasons.push({
