...
export function getExtraTags(r) {
  ...
  if (isKidsProduct(r)) tags.push('Kids');
...
export const OUTPUT_COLUMNS = [
  ...
  'product.metafields.custom.age_group',
];
...
export const JEWEL_STYLE_HEADER = 'Metafield: custom.jewel_style (list.single_line_text_field)';
export const STONES_HEADER      = 'Metafield: custom.stones (list.single_line_text_field)';

// Shopify list-metafield imports require one value per product row. The first
// row carries Handle + Title; continuation rows carry Handle only.
export const JEWEL_STYLE_UPDATE_COLUMNS = ['Handle', 'Title', JEWEL_STYLE_HEADER];
export const STONES_UPDATE_COLUMNS      = ['Handle', 'Title', STONES_HEADER];
  ...
  return rows
    ...
    .map(r => {
      ...
      return {
        ...
        'product.metafields.custom.age_group': isKidsProduct(r) ? 'Kids' : 'Adults',
        'product.metafields.custom.stones': getStoneValues(r).join('|'),
      };
    })
    ...
    .map(r => {
      ...
      // Retained internally to generate the dedicated one-value-per-row
      // list-metafield import; deliberately excluded from the variant CSV.
      out['product.metafields.custom.jewel_style'] = r['product.metafields.custom.jewel_style'] ?? '';
      out['product.metafields.custom.stones'] = r['product.metafields.custom.stones'] ?? '';
      return out;
    })
...
export function buildListMetafieldUpdateRows(rows, sourceColumn, metafieldHeader) {
  return dedupeByHandle(rows).flatMap(product => {
    const values = String(product[sourceColumn] || '').split('|').filter(Boolean);
    if (!values.length) values.push(''); // blank row clears a stale existing value
    return values.map((value, index) => ({
      Handle: product['URL Handle'],
      Title: index === 0 ? product.Title : '',
      [metafieldHeader]: value,
    }));
  });
}
      ...
      complete: ({ data }) => {
        ...
        setTimeout(() => {
          try {
            ...
            // Description + Tags + Age Group update files: all variant rows.
            ...
            // Dedicated Shopify list-metafield import: one value per product row,
            // with Handle repeated and Title present only on the first row.
            const jewelStyleRows = buildListMetafieldUpdateRows(
              transformed, 'product.metafields.custom.jewel_style', JEWEL_STYLE_HEADER
            );
            const stoneRows = buildListMetafieldUpdateRows(
              transformed, 'product.metafields.custom.stones', STONES_HEADER
            );
            const jewelStyleChunks = chunkByURLHandle(
              jewelStyleRows, 15, JEWEL_STYLE_UPDATE_COLUMNS, 'Handle'
            );
            const stoneChunks = chunkByURLHandle(
              stoneRows, 15, STONES_UPDATE_COLUMNS, 'Handle'
            );
            ...
            const descFiles = descChunks.map((rows, i) => ({
              label:    `Description + Tags + Age Group Update — Part ${i + 1} of ${descChunks.length}`,
              filename: `${baseName}_desc_update_${i + 1}.csv`,
              rows,
              columns:  DESC_UPDATE_COLUMNS,
            }));

            const metafieldFiles = [
              ...jewelStyleChunks.map((rows, i) => ({
                label:    `Jewel Style List — Part ${i + 1} of ${jewelStyleChunks.length}`,
                filename: `${baseName}_jewel_style_${i + 1}.csv`,
                rows,
                columns:  JEWEL_STYLE_UPDATE_COLUMNS,
              })),
              ...stoneChunks.map((rows, i) => ({
                label:    `Stones List — Part ${i + 1} of ${stoneChunks.length}`,
                filename: `${baseName}_stones_${i + 1}.csv`,
                rows,
                columns:  STONES_UPDATE_COLUMNS,
              })),
            ];
