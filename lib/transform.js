...
export function getStoneValues(r) {
  ...
  if ((!suppN && !prodN) || suppN.includes('nogem') || suppN.replace(/\s/g, '').includes('toutmetal')) return [];
...
export const OUTPUT_COLUMNS = [
  ...
  'product.metafields.custom.stones',
];

// Minimum columns to update Description, Tags, and custom metafields on existing
      ...
      complete: ({ data }) => {
        ...
        setTimeout(() => {
          try {
            ...
            // Description + Tags + Metafields update files: all variant rows, 11 columns.
