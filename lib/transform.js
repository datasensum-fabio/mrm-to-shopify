...
function getGemstone(supp, prod) {
  ...
  // Detect colour qualifiers (e.g. oz.blu → Blue CZ, topa.ros → Pink Topaz)
  const combinedN    = norm(combined);
  const colourPat    = Object.keys(STONE_COLOUR_MAP).map(k => k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|');
  ...
  for (const [abbrev, gemVal] of Object.entries(GEM_ABBREV_TO_VAL)) {
    ...
    const m   = combinedN.match(new RegExp(`\\b${esc}[.\\s\\-]*(${colourPat})\\b`));
    if (m) gemColours[gemVal] = STONE_COLOUR_MAP[m[1]];
  }
  const gemStrs = gems.map(g => {
    const base   = g.replace('Synthetic ', '').replace('Natural ', '');
    const colour = gemColours[base] || '';
...
function getStyle(libProd, libSupp) {
  ...
  if (letterM) {
    ...
    const zodIdx = seen.indexOf('zodiac');
    if (zodIdx !== -1) seen.splice(zodIdx, 1);
  }
