...
// Keep the Jewel Style metafield intentionally small and shopper-facing.
// General merchandising tags and individual stone names remain in Tags and
// custom.stones, but must not become Jewel Style filter options.
export function getJewelStyleValues(r, extraTags = getExtraTags(r), stones = getStoneValues(r)) {
  const values = [];
  const add = value => { if (!values.includes(value)) values.push(value); };
  const isPlainMetal = stones.includes('Plain Metal/No Gems');
  const isPearl = stone => stone === 'Pearl' || stone.endsWith(' Pearl');
  const hasPearl = stones.some(isPearl);
  const hasStone = stones.some(stone => stone !== 'Plain Metal/No Gems' && !isPearl(stone));

  // Every product gets a base Jewel Style. Pearls are classified separately
  // from stones; mixed pearl-and-gem pieces receive both classifications.
  if (isPlainMetal) add('Plain Metal');
  if (hasStone) add('Stone Set');
  if (hasPearl) add('Pearl');

  if (extraTags.includes('ONLY ZIRCONIA')) add('Only Zirconia');
  if (extraTags.includes('Hoop Earring')) add('Hoop Earrings');
  if (extraTags.includes('Kids')) add('For Kids');
  if (extraTags.includes('SIGNET RING')) add('Signet Ring');
  if (extraTags.includes('BANGLE')) add('Bangle');
  if (extraTags.includes('GENTS')) add('For Men');
  return values;
}
...
export function isCollectionEligible(r) {
  const tags = (r.Tags || '').toLowerCase();
  const metal = r['Option2 Value'] || '';
  const isWeddingRing = /wedding ring/i.test(tags);
  const is18ctWeddingRing =
    metal.startsWith('18CT') && isWeddingRing;
  const isEligibleAGirardWeddingRing =
    tags.includes('a.girard') &&
    isWeddingRing &&
    (metal.startsWith('9CT') || metal.startsWith('18CT'));
  const isEligibleMRM750Ring =
    (tags.includes('mrm 750') || tags.includes('mrm 750emp')) &&
    r.Type === 'Rings';
  return is18ctWeddingRing ||
    isEligibleAGirardWeddingRing ||
    isEligibleMRM750Ring ||
    COLLECTION_TAGS.some(tag => tags.includes(tag));
}
...
export const DESC_UPDATE_COLUMNS = [
  'Title', 'URL Handle', 'Description', 'Tags', 'Published on online store', 'Status',
...
function transform(rows, dropReasons = []) {
  ...
  return rows
    ...
    .map(r => {
      ...
      const stones = getStoneValues(r);
      return {
        ...r,
        Tags: extra.length ? r.Tags + ', ' + extra.join(', ') : r.Tags,
        [JEWEL_STYLE_HEADER]: getJewelStyleValues(r, extra, stones).join('\n'),
        [AGE_GROUP_HEADER]: isKidsProduct(r) ? 'Kids' : 'Adults',
        [STONES_HEADER]: stones.join('\n'),
      };
    })
    ...
    // Step 23: products outside the approved collection rules remain excluded
    // when they are new or no Shopify match was supplied. When a product already exists in
    // Shopify, retain it as a draft/unpublished row so obsolete listings are
    // taken offline and their product-level fields can still be corrected.
    .map(r => ({ ...r, _collectionEligible: isCollectionEligible(r) }))
    .filter(trackDrop(
      'collection_not_approved',
      'Product is not in an approved collection or an eligible collection/type exception',
      r => r._collectionEligible || r._shopifyExistingVariant
    ))
    .map(r => r._collectionEligible ? r : {
      ...r,
      Status: 'draft',
      'Published on online store': false,
    })

    // Step 24: select and reorder output columns
    .map(r => {
      const out = {};
...
export async function expandShopifyInputFiles(files) {
  const expanded = [];
  for (const file of files) {
    ...
    if (!lowerName.endsWith('.zip')) {
      throw new Error(`Unsupported Shopify export file: ${file.name}. Upload .csv or .zip files.`);
    }

    let entries;
    try {
      entries = await unzipEntries(file);
    } catch (error) {
      throw new Error(`Could not read Shopify ZIP ${file.name}: ${error.message || 'invalid ZIP archive'}`);
    }
    const csvEntries = Object.entries(entries).filter(([name]) =>
      !name.endsWith('/') && name.toLowerCase().endsWith('.csv')
    );
    if (csvEntries.length !== 1) {
      throw new Error(
        `Shopify ZIP ${file.name} must contain exactly one CSV file; found ${csvEntries.length}.`
      );
    }
    const [entryName, bytes] = csvEntries[0];
    const csvName = entryName.split('/').pop();
    expanded.push(new File([bytes], csvName, {
      type: 'text/csv',
...
export function getShopifyHandleIndex(rows) {
  const first = rows[0] || {};
  const handleColumn = Object.prototype.hasOwnProperty.call(first, 'Handle')
    ? 'Handle'
    : Object.prototype.hasOwnProperty.call(first, 'URL Handle') ? 'URL Handle' : null;
  ...
  for (const row of rows) {
    const handle = String(row[handleColumn] || '').trim();
    const key = canonicalSKU(handle);
    if (key) handles.set(key, handle);
  }
  if (!handles.size) throw new Error('The Shopify export does not contain any product handles.');
  return handles;
}

export function getShopifyHandleSet(rows) {
  return new Set(getShopifyHandleIndex(rows).keys());
}
...
export function matchMRMRowsToShopifyProducts(rows, shopifyVariantIndex, shopifyHandleIndex) {
  return rows.flatMap(row => {
    const sku = canonicalSKU('MRM-' + String(row.code_variant || '').trim());
    const expectedHandle = canonicalSKU('mrm' + String(row.code_produit || '').trim());
    const shopifyHandle = shopifyVariantIndex.get(sku) || shopifyHandleIndex.get(expectedHandle);
    if (!shopifyHandle) return [];
    return [{
      ...row,
      _shopifyExistingVariant: true,
      _shopifyHandle: shopifyHandle,
    }];
  });
}
...
const SHOPIFY_COMPARE_FIELDS = [
  ...
  { output: 'Status', sources: ['Status'], normalize: value => normalizeText(value).toLowerCase() },
...
export function getDescriptionUpdateRows(transformedRows, shopifyRows) {
  ...
  const productKeyFor = row => {
    const skuMatch = skuToProduct.get(canonicalSKU(row.SKU));
    if (skuMatch) return skuMatch;
    const handleMatch = canonicalSKU(row['URL Handle']);
    return products.has(handleMatch) ? handleMatch : '';
  };
  for (const row of transformedRows) {
    const productKey = productKeyFor(row);
  ...
  return transformedRows.filter(row =>
    changedProducts.has(productKeyFor(row))
  );
}

export async function processFile(file, shopifyInput = null) {
  const selectedShopifyFiles = (Array.isArray(shopifyInput) ? shopifyInput : shopifyInput ? [shopifyInput] : [])
    .filter(Boolean);
  const shopifyFiles = await expandShopifyInputFiles(selectedShopifyFiles);
    ...
    setTimeout(() => {
      try {
            ...
            if (shopifyParsed) {
              ...
              const shopifyHandleIndex = getShopifyHandleIndex(shopifyParsed.data);
              const shopifyHandles = new Set(shopifyHandleIndex.keys());
              ...
              sourceRows = matchMRMRowsToShopifyProducts(data, shopifyVariantIndex, shopifyHandleIndex);
              dropReasons.push({
                key: 'not_in_shopify_export',
                label: 'Neither variant SKU nor expected product handle is present in the supplied Shopify export',
            ...
            unpublishedRows = transformed.filter(row => !row['Published on online store']).length;
            unpublishedProducts = new Set(
              transformed
                .filter(row => !row['Published on online store'])
                .map(row => row['URL Handle'])
                .filter(Boolean)
            ).size;
