...
export function getJewelStyleValues(r, extraTags = getExtraTags(r), stones = getStoneValues(r)) {
  ...
  const hasPearl = stones.some(stone => stone === 'Pearl' || stone.endsWith(' Pearl'));

  // Every product gets a base Jewel Style: no-gem pieces are Plain Metal and
  // every piece with at least one positively identified gem is Stone Set.
  ...
  if (extraTags.includes('Stone Set Earring')) add('Stone Set Earrings');
