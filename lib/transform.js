...
function buildDescription(r) {
  ...
  else            items.push(`<li><b>Gemstone:</b> Plain metal</li>`);
...
// Minimal columns needed to update the Description field on existing Shopify products.
// One row per product (handle) — Description is product-level, not variant-level.
export const DESC_UPDATE_COLUMNS = ['Title', 'URL Handle', 'Description'];
      ...
      complete: ({ data }) => {
        ...
        setTimeout(() => {
          try {
            ...
            // Description-only update files: one row per product, 3 columns
            const descRows         = dedupeByHandle(transformed);
            const descChunks       = chunkByURLHandle(descRows, 15, DESC_UPDATE_COLUMNS);
            ...
            const descFiles = descChunks.map((rows, i) => ({
              label:    `Description Update — Part ${i + 1} of ${descChunks.length}`,
