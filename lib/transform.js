...
const COLLECTION_TAGS = [
  ...
  'mrm 750', 'mrm 750emp', 'a.girard',
];

export function isCollectionEligible(r) {
  const tags = (r.Tags || '').toLowerCase();
  const is18ctWeddingRing =
    (r['Option2 Value'] || '').startsWith('18CT') &&
    /wedding ring/i.test(tags);
  return is18ctWeddingRing || COLLECTION_TAGS.some(tag => tags.includes(tag));
}
...
function transform(rows, dropReasons = []) {
  ...
  return rows
    ...
    // Step 24: every row must have an approved collection tag. The sole
    // exception is an 18CT wedding ring, for which every collection is allowed.
    .filter(trackDrop(
      'collection_not_approved',
      'Product is not in an approved collection (except 18CT wedding rings)',
